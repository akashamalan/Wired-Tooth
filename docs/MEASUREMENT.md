...
1. Play the click track generated by `tools/acoustic.py generate`. Each click
   is a 3 ms chirp sweeping 500 Hz to 8 kHz.

   **Not a 1 kHz tone burst**, which is what this document originally said. A
   pure tone is periodic, so cross-correlating against it produces a peak
   every 1 ms and the result is ambiguous by whole milliseconds -- the same
   order as the thing being measured. Reading the onset by eye in Audacity has
   the same ambiguity and worse precision. A chirp has a tone burst's energy
   and a single sharp correlation peak.
...
4. Record a **reference pass first**, with the earbud silent, so the analyser
   learns the room's reflections. A click bouncing off a desk lands 5-15 ms
   behind the direct sound, which is the same range as a good result, so
   without this the two are indistinguishable.
5. Analyse:

       python tools/acoustic.py measure recording.wav --reference reference.wav

   It cross-correlates against the known chirp, groups arrivals per emitted
   click, discards offsets that also appear in the reference, and reports the
   median, range and standard deviation over all 10 clicks.

   Doing this by hand in Audacity also works -- select from the first
   transient to the second, set the toolbar to **Samples**, and compute
   `samples / sample_rate * 1000`. It is slower and it does not protect you
   from picking a reflection.
